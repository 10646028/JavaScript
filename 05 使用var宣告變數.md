...
```
在函式內的if指令中宣告var變數, 意指[重新宣告一次函式內的區域變數], 很容易造成不預期的問題.
建議在if, for等指令範圍內, 使用let指令宣告該指令範圍內的區域變數.
```
